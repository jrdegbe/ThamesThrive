...
from ThamesThrive.service.storage.driver.elastic import identification as identification_db

if License.has_service(LICENSE):
    from com_ThamesThrive.bridge.bridges import javascript_bridge
    from com_ThamesThrive.service.browser_fingerprinting import BrowserFingerPrint
...
class TrackerPayload(BaseModel):
    ...
    _ThamesThrive_referer: dict = PrivateAttr({})
    ...
    def __init__(self, **data: Any):
        ...
        super().__init__(**data)
        self._id = str(uuid4())
        self._ThamesThrive_referer = self.get_ThamesThrive_data_referer()
    ...
    @property
    def ThamesThrive_referer(self):
        return self._ThamesThrive_referer
    ...
    def is_debugging_on(self) -> bool:
        return ThamesThrive.track_debug and self.is_on('debugger', default=False)
    ...
    def get_ThamesThrive_data_referer(self) -> dict:

        try:
            return self.context['ThamesThrive']['pass']
        except (KeyError, TypeError):
            return {}

    def get_referer_data(self, type: str) -> Optional[str]:
        if self._ThamesThrive_referer:
            if type in self._ThamesThrive_referer:
                return self._ThamesThrive_referer[type].strip()
        return None
            ...
            for field in _identification.fields:
                if field.event_property.ref and field.profile_trait.ref:
                    if field.event_property.value not in flat_properties:
                        raise AssertionError(f"While creating new profile ThamesThrive was forced to load data by merging "
