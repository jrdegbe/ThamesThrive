...
from ThamesThrive.service.tracking.storage.profile_storage import load_profile
from ThamesThrive.service.utils.date import now_in_utc
from ThamesThrive.service.profile_merger import ProfileMerger
...
from ThamesThrive.service.logger_manager import save_logs
from ThamesThrive.service.setup.data.defaults import open_rest_source_bridge
from ThamesThrive.service.tracking.source_validation import validate_source
from ThamesThrive.service.storage.driver.elastic.operations import console_log as console_log_db
...
from ThamesThrive.service.tracking.track_async import process_track_data
...
class Tracker:
        ...
        if refer_source_id is not None and tracker_payload.has_referred_profile():
            ...
            try:
                ...
                if referred_profile is not None:
                    ...
                    logger.warning(f"Referred ThamesThrive Profile Id {referred_profile_id} is invalid.")

            except ValueError as e:
                logger.warning(f"Referred ThamesThrive Source Id {refer_source_id} is invalid. "
    ...
    async def track_event(self, tracker_payload: TrackerPayload, tracking_start: float):
        ...
        # ----------------------------------
        # Validate ThamesThrive referer. ThamesThrive referer is a data that has profile_id, session_id. source_id.
        ...
        if result and ThamesThrive.enable_errors_on_response:
    ...
    async def check_source_id(self, source_id) -> Optional[EventSource]:
        ...
        if not ThamesThrive.enable_event_source_check:
            return EventSource(
                id=source_id,
                type=['rest'],
    ...
    async def validate_source(self, tracker_payload: TrackerPayload) -> EventSource:
        ...
        if source_id == ThamesThrive.internal_source:
            return EventSource(
                id=source_id,
                type=['internal'],
                bridge=NamedEntity(id=open_rest_source_bridge.id, name=open_rest_source_bridge.name),
