...
from ThamesThrive.context import ServerContext, get_context, Context
...
from ThamesThrive.service.setup.setup_indices import create_schema, install_default_data, run_on_start
from ThamesThrive.service.storage.driver.elastic import raw as raw_db
from ThamesThrive.service.storage.driver.elastic import system as system_db
...
if License.has_license() and License.has_service(MULTI_TENANT):
    from com_ThamesThrive.service.multi_tenant_manager import MultiTenantManager
...
async def check_installation():
    ...
    if ThamesThrive.multi_tenant and (not is_schema_ok or not has_admin_account):
        if License.has_service(MULTI_TENANT):
            ...
            try:
                await mtm.authorize(ThamesThrive.multi_tenant_manager_api_key)
...
async def install_system(credentials: Credentials):
    if ThamesThrive.multi_tenant:
        ...
        if ThamesThrive.installation_token and ThamesThrive.installation_token != credentials.token:
            raise PermissionError("Installation forbidden. Invalid installation token.")
    ...
    with ServerContext(get_context().switch_context(production=False)):
        ...
        if os.environ.get("DEMO", 'no') == 'yes':
            ...
            for i in range(0, 100):
                payload = generate_payload(source=ThamesThrive.demo_source)
