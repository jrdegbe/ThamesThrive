...
from ThamesThrive.service.tracking.storage.event_storage import save_events
from ThamesThrive.service.tracking.storage.profile_storage import save_profile, load_profile
from ThamesThrive.service.tracking.storage.session_storage import save_session
...
from ThamesThrive.exceptions.exception import StorageException, FieldTypeConflictException
...
class TrackingPersisterAsync:
    ...
    async def save_events(self, events: List[Event]):
        if ThamesThrive.expose_gui_api is True:
            if events:
