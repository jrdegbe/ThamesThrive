...
from ThamesThrive.domain.migration_schema import MigrationSchema, CopyIndex
...
from ThamesThrive.worker.celery_worker import run_migration_job
import asyncio
...
class MigrationManager:
    ...
    async def get_available_schemas(self) -> Dict[str, Union[bool, List[MigrationSchema]]]:
        ...
        if self.get_current_db_version_prefix(ThamesThrive.version) != self.to_version or tenant != self.to_tenant:
            raise ValueError(f"Installed system version is {ThamesThrive.version.db_version} for tenant {tenant}, "
        ...
        warn = self.from_version in ThamesThrive.version.upgrades
    ...
    async def save_version_update(self):
        ...
        ThamesThrive.version.add_upgrade(self.from_version)
